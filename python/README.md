...
- The gear in the top right, or ⌘, opens **settings** in a window of its own: how long a session runs (1 to 180 minutes) and the hour a new day begins. Each session stores the length it started with, so changing it affects only sessions started afterwards — a running session and every past one keep theirs.
- Closing the main window keeps the app running. Use its Dock icon or **Window → Show lock in no gooning** to reopen it. **⌘Q** quits.

Dates are grouped by session start time in the Mac’s current local time zone, with a day running 8am to 8am. A session started at 4am is filed under the previous date, and “today” means the same thing, so a late night stays on the day it began. The boundary defaults to 8am and is set in settings. The calendar starts on Sunday. Native controls and semantic system colors follow light and dark appearance. The palette follows macOS Calendar and uses a single accent: red marks today, logged entries, and the running clock, while the selected day takes a neutral fill, so the only saturated color carries meaning. The clock itself is primary text — white in dark appearance. Session and goal rows sit on a recessed card rather than a lit bezel, and a completed goal is marked with a red symbol instead of a system-accent checkbox.
