...
A small, offline macOS 14+ app: one window, a timer (20 minutes by default), daily goals, and a calendar of what you did. Written in Python with native AppKit controls through PyObjC and a local SQLite database. No backend, account, or network requests.
...
- The gear in the top right, or ⌘, opens settings in its own window: the session length and the hour a new day starts. A session records the length it began with, so changing it never rewrites past sessions.
- **Start** begins and immediately persists a session. **End Session** stops it early and opens the note sheet.
...
Sessions and daily goals live in the same database. Dates are absolute Unix timestamps, grouped by session start time in the Mac's current local time zone. A day runs 8am to 8am by default, so work done at 4am is filed under the previous date rather than starting a new one.
